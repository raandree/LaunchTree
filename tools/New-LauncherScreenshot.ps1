...
try {
    ...
    # Enough populated Menu Folders that the tab strip overflows and must scroll.
    foreach ($folderName in 'Games', 'Photo editing', 'Streaming services',
        'Productivity suites', 'Video conferencing') {
        $extraFolder = New-Item -Path (Join-Path $entertainment.FullName $folderName) -ItemType Directory
        @('[InternetShortcut]', 'URL=https://www.example.com/') |
            Set-Content -LiteralPath (
                Join-Path $extraFolder.FullName "$folderName.url"
            ) -Encoding ASCII
    }
